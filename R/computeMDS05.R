computeMDS05 <- function (data, Vegetables, Legumes, FruitAndNuts, Cereals, Potatoes = NULL, Fish, Meat, Dairy, Alcohol,
  ...
  Fiscore[Sex == men & Fish < Me(Fish[Sex == men])] <- 0
  Fiscore[Sex == men & Fish >= Me(Fish[Sex == men])] <- 1
  Fiscore[Sex == women & Fish < Me(Fish[Sex == women])] <- 0
  Fiscore[Sex == women & Fish >= Me(Fish[Sex == women])] <- 1
  ...
  Fatscore <- rep(NA, length = nrow(data))
  FATS <- (MUFA + PUFA) / SFA
  ...
  if(!is.null(Fats) && (!is.null(MUFA) || !is.null(PUFA) || !is.null(SFA))) {
    FATS <- Fats
    warning("To compute the score, the 'Fats' argument has been used, but redundandt arguments ('MUFA', 'PUFA' or 'SFA') have been provided,
